...
/*
void Socket::handling_messages(vector<Socket> *reads, int master_socket) {
    int id = reads->at(0).sock;
    struct timeval tv;
    fd_set readfds;
    ...
    int result = ::select(maxSock+1, &readfds, nullptr, nullptr, &tv);
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                for(int i = 0; i < reads->size(); ++i) {
                    int outSock = reads->at(i).sock;
                    if(master_socket != outSock && outSock != sd) {
						cout << "send msg" << endl;
                        ostringstream ss;
                        ss << "SOCKET #" << sd << ": " << buffer << "\r\n";
